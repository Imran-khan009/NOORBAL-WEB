  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        ...
        <div className="w-screen max-w-md bg-white shadow-2xl border-l border-[#E5DFD5] flex flex-col justify-between">
          
          {/* Header */}
          <div className="p-5 border-b border-[#E5DFD5] flex items-center justify-between bg-[#FAF8F5]">
            <div className="flex items-center gap-2">
              <Heart className="w-5 h-5 text-rose-600 fill-rose-600" />
              <h3 className="font-serif text-lg font-bold text-[#2B231E]">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-full hover:bg-black/5 text-[#2B231E]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h4 
                        ...
                        className="font-serif text-sm font-bold text-[#2B231E] line-clamp-1 cursor-pointer hover:text-[#C9A468]"
                      >
                        {item.name}
                      </h4>
                      <p className="text-xs text-[#8D7B68] mt-0.5">
                        {item.status === 'ready-stock' ? 'Ready-Stock (2–3 Days)' : 'Made-to-Order'}
                      </p>
                      <p className="font-serif text-sm font-bold text-[#2B231E] mt-1">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <button
                        ...
                        className="text-xs font-semibold text-[#2B231E] hover:text-[#C9A468] flex items-center gap-1.5 transition-colors"
                      >
          ...
          {/* Footer Checkout */}
          {wishlist.length > 0 && (
            <div className="p-5 border-t border-[#E5DFD5] bg-[#FAF8F5] space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-[#5C5046]">Estimated Total:</span>
                <span className="font-serif text-lg font-bold text-[#2B231E]">
                  {formatPrice(
              ...
              <button
                ...
                className="w-full py-3 bg-[#25D366] hover:bg-[#20ba5a] text-white font-sans text-xs uppercase tracking-wider font-bold rounded-xl shadow transition-all flex items-center justify-center gap-2"
              >
